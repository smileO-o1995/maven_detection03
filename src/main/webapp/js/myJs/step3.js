        ...
        $.post("detectionServlet", $("#myStep3Form").serialize(), function (data) {
            ...
            if(!obj.sta){
                ...
                if(!resData.stat){//定性检查错误
                    $("#myStep3_2").show();
                    $(".errResult").append(resData.msg);
                    $(".noteStep3").append("error");
                }else{//定性检查正确
                    //网表是否为木马网表
                    if(resData.normal){//为普通网表
                        $("#myStep3_2").show();
                        $(".errResult").append(resData.msg);
                        $(".noteStep3").append("right");
                    }else{
                        $("#myStep3_3").show();
                        ...
                        for(var key in netSet){
                            if(key != len ){
                                $(".netSet").append(netSet[key] + ",  ");
                            }else {
                                $(".netSet").append(netSet[key]);
                            }
                        }
                        $(".successRate").append(resData.successRate);
                        $(".errorRate").append(resData.errorRate);
                        $(".noteStep3").append("pass");
                    }
                }
